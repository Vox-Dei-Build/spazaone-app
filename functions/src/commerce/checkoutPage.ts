...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    label{display:block;font-size:13px;font-weight:750;margin-bottom:6px}input{width:100%;border:1px solid #b9c8c2;border-radius:12px;padding:13px 12px;font:inherit;outline:none}input:focus{border-color:var(--green);box-shadow:0 0 0 3px rgba(16,108,85,.12)}
    ...
    <section class="card">
      ...
      <form id="checkoutForm">
        <div class="grid">
          ...
          <div class="wide"><button id="payButton" type="submit">${actionLabel}</button><div class="secure">${paymentNotice}</div></div>
        </div>
      </form>
    </section>`;
  const script = `<script>
    ...
    form.addEventListener('submit',async event=>{
      ...
      const body={listingId:config.listingId,checkoutAttemptId:attempt,buyer:{name:value('name'),email:value('email'),phone:value('phone')},deliveryAddress:{line1:value('line1'),line2:value('line2'),suburb:value('suburb'),city:value('city'),province:value('province'),postalCode:value('postalCode'),country:'ZA'}};
      try{
      ...
      }catch(error){errorBox.textContent=error.message||'Order could not be submitted.';errorBox.style.display='block';button.disabled=false;button.textContent=${safeJson(actionLabel)};errorBox.scrollIntoView({behavior:'smooth',block:'center'});}
    });
  </script>`;
