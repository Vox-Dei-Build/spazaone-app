...
/**
 * Idempotently prepares one Pasella-owned product promotion template and a
...
export const ensureProductPromotionTemplate = functions.https.onCall(
  async (data: EnsureRequest, context) => {
    ...
    await db.runTransaction(async (tx) => {
      ...
      // Pasella's shared template was submitted manually in Twilio so its
    ...
    if (!latest.exists || !latestData) {
      throw new functions.https.HttpsError(
        "internal",
        "Pasella could not prepare the promotion message.",
      );
    }
