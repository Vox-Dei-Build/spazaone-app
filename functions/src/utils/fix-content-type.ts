...
export const fixProductsContentType = functions
  ...
  .https.onRequest(async (req, res): Promise<void> => {
    try {
      ...
      const concurrency = Math.max(1, Math.min(20, parseInt(String(req.query.concurrency || '10'), 10) || 10))

      if (!bucketName) { res.status(400).json({ ok:false, error:'Missing query param: bucket' }); return }
      ...
      const results = await mapWithLimit(files, concurrency, async (f) => {
        ...
        const current = (meta.contentType || 'application/octet-stream').toLowerCase()

        // Update only if needed
        const needs = current !== targetType || current === 'application/octet-stream'
        if (!needs) {
          return { file: name, updated: false, from: current, to: targetType, reason: 'Already correct' }
        }

        if (dryRun) {
          return { file: name, updated: false, from: current, to: targetType, reason: 'Dry run' }
        }
