  ...
  static Stream<Map<String, dynamic>> orderStream({required String orderId}) {
    ...
    return doc.snapshots().map((snap) {
      ...
      return {
        ...
        'orderRequest': s['orderRequest'] == true,
        'orderChannel': s['orderChannel'] ?? '',
        'fulfillmentType': s['fulfillmentType'] ?? '',
        'deliveryAddress': s['deliveryAddress'] ?? '',
        'deliveryInfo': s['deliveryInfo'] ?? '',
        'requestedFulfillmentTime': s['requestedFulfillmentTime'] ?? '',
        'cashChangeFor': s['cashChangeFor'],
        'customerNote': s['customerNote'] ?? s['remarks'] ?? '',
        'driver': (s['driver'] is Map) ? s['driver'] : const {},
